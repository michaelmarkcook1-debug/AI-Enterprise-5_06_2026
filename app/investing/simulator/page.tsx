...
export default async function InvestmentSimulatorPage() {
  ...
  return (
    ...
    >
      ...
      {live && (
        <div className="mb-5 rounded-xl border border-[#dfe4da] bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-semibold uppercase tracking-wider text-[#697362] dark:text-zinc-500">
              ...
              <p className="mt-1 text-sm text-[#18201b] dark:text-zinc-100">{live.summary}</p>
              <p className="mt-1 text-[11px] text-[#697362] dark:text-zinc-500">
            ...
            <div className="text-right">
              <div className="text-[10px] uppercase tracking-wider text-[#697362] dark:text-zinc-500">Annualised tilt</div>
          ...
          <Panel title="Live news + AI Atlas context for the seed portfolio holdings">
            ...
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
              {live.holdings.map((h) => {
                ...
                return (
                  <div key={h.providerId} className="rounded-md border border-[#dfe4da] bg-[#fafbf8] p-3 dark:border-zinc-800 dark:bg-zinc-900/60">
                    <div className="flex items-center justify-between gap-2">
                      <Link
                        href={`/investor-tools/provider/${h.providerId}`}
                        className="font-semibold text-[#18201b] hover:underline dark:text-zinc-100"
                      >
                        {h.providerName}
                      </Link>
                      <span className={`font-mono text-xs font-semibold ${
                        ...
                        : "text-[#697362] dark:text-zinc-500"
                      }`}>
                        {h.tilt.tilt >= 0 ? "+" : ""}{tiltPp}pp
                      </span>
                    </div>
                    <div className="mt-1 flex flex-wrap items-center gap-2 text-[10px] text-[#697362]">
                      {atlas ? (
                      ...
                      {uptake !== null && (
                        <span className="rounded-full bg-[#eef2e8] px-1.5 py-0.5 font-semibold text-[#455044] dark:bg-zinc-800 dark:text-zinc-300">
                    ...
                    {news.length === 0 ? (
                      <p className="mt-2 text-[11px] italic text-[#66705f]">No recent classified news.</p>
                    ) : (
                      <ul className="mt-2 space-y-1.5">
                        {news.slice(0, 2).map((n) => (
                          <li key={n.id} className="text-[11px] leading-4">
                            <div className="font-medium text-[#18201b] dark:text-zinc-200">{n.title}</div>
                            <div className="text-[10px] text-[#66705f]">{new Date(n.publishedAt).toLocaleDateString()} · impact {n.impactScore}</div>
                          </li>
                        ))}
                      </ul>
                    )}
