    ...
    <div className="rounded-lg border border-[#e6dcc3] bg-white p-4 dark:border-[#1d3a57] dark:bg-[#071827]">
      ...
      {provider ? (
        <>
          ...
          <p className="mt-3 text-xs leading-5 text-[#54647a] dark:text-[#a7bacd]">{reason ?? provider.keyThesis}</p>
          <p className="mt-2 text-xs leading-5 text-[#7a5847] dark:text-amber-300">Risk: {risk ?? provider.mainRisk}</p>
        </>
      ) : (
        <div className="mt-3 text-sm text-[#54647a] dark:text-[#a7bacd]">No seed signal available.</div>
      )}
    </div>
  );
