    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-[#dfe4da] bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            <h3 className="text-sm font-semibold text-[#18201b] dark:text-zinc-100">Filters</h3>
            <p className="text-xs text-[#5f685a] dark:text-zinc-400">
          ...
          {(selectedRegions.length > 0 || selectedIndustries.length > 0 || selectedSize) && (
            <button
              type="button"
              onClick={clearAll}
              className="rounded-md border border-[#dfe4da] bg-white px-3 py-1 text-xs font-semibold text-[#18201b] hover:bg-[#eef2e8] dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100 dark:hover:bg-zinc-800"
            >
              Clear all
            </button>
          )}
      ...
      {headlineTop3.length > 0 && (
        <div className="grid gap-3 sm:grid-cols-3">
          {headlineTop3.map((row, i) => (
            <div key={row.vendor} className="rounded-xl border border-[#dfe4da] bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
              <div className="flex items-center justify-between gap-2">
                <span className="text-[10px] font-semibold uppercase tracking-wider text-[#697362] dark:text-zinc-500">#{i + 1}</span>
                <span className={`inline-flex items-center gap-1 text-[10px] font-medium text-[#5f685a] dark:text-zinc-400`}>
              ...
              <div className="mt-1 flex items-center gap-2">
                ...
                <span className="text-base font-semibold text-[#18201b] dark:text-zinc-100">{row.vendor}</span>
              </div>
              <div className="mt-1 font-mono text-2xl font-semibold text-[#18201b] dark:text-zinc-100">{pct(row.share)}</div>
              <div className="mt-0.5 text-[11px] text-[#697362] dark:text-zinc-500">share inside this slice</div>
            </div>
          ))}
        </div>
      )}
      ...
      {/* Tab switcher */}
      <div className="flex gap-1 border-b border-[#dfe4da] dark:border-zinc-800">
...
function FilterGroup({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mt-3 first:mt-0">
      <div className="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-[#697362] dark:text-zinc-500">{title}</div>
    ...
    <button
      ...
      className={`rounded-full border px-3 py-1 text-xs font-medium transition-colors ${
        active
          ...
          : "border-[#dfe4da] bg-white text-[#18201b] hover:bg-[#eef2e8] dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200 dark:hover:bg-zinc-800"
      }`}
    >
      {children}
    </button>
...
function VendorBarChart({ rows }: { rows: UptakeAggregateRow[] }) {
  if (rows.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-[#dfe4da] bg-[#fafbf8] p-8 text-center text-sm text-[#5f685a] dark:border-zinc-700 dark:bg-zinc-900/50 dark:text-zinc-400">
      ...
      <ul className="space-y-2">
        {rows.map((r) => (
          <li key={r.vendor} className="grid grid-cols-[140px_1fr_70px_80px] items-center gap-3">
            <div className="flex items-center gap-2 truncate text-sm font-medium text-[#18201b] dark:text-zinc-100">
              <span className="h-3 w-3 flex-none rounded-sm" style={{ background: colorFor(r.vendor) }} />
              <span className="truncate">{r.vendor}</span>
            </div>
            <div className="relative h-5 rounded-full bg-[#eef2e8] dark:bg-zinc-800">
              <div
                className="h-full rounded-full transition-all"
                style={{ width: `${max > 0 ? (r.share / max) * 100 : 0}%`, background: colorFor(r.vendor) }}
              />
            </div>
            <div className="text-right font-mono text-sm font-semibold text-[#18201b] dark:text-zinc-100">{pct(r.share)}</div>
            <div className="flex items-center justify-end gap-1 text-[10px] text-[#5f685a] dark:text-zinc-400">
  ...
  return (
    <div className="rounded-xl border border-[#dfe4da] bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
      <div className="mb-3 flex flex-wrap items-center gap-3">
        <span className="text-xs font-semibold uppercase tracking-wider text-[#697362] dark:text-zinc-500">Vendor focus</span>
        <select
          value={vendor}
          onChange={(e) => setVendor(e.target.value)}
          className="rounded-md border border-[#dfe4da] bg-white px-3 py-1.5 text-sm font-medium text-[#18201b] dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
        >
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-xs">
          <thead>
            <tr>
              <th className="px-2 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-[#697362] dark:text-zinc-500"></th>
              {INDUSTRIES.map((i) => (
                <th key={i} className="px-2 py-2 text-left text-[10px] font-medium text-[#5f685a] dark:text-zinc-400">{shortIndustry(i)}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {REGIONS.map((r) => (
              <tr key={r} className="border-t border-[#edf0ea]/60">
                <td className="whitespace-nowrap px-2 py-2 text-xs font-semibold text-[#18201b] dark:text-zinc-100">{r}</td>
                {INDUSTRIES.map((i) => {
                  const v = matrix[r]?.[i] ?? 0;
                  return (
                    <td key={i} className="px-1 py-1 text-center align-middle">
                      <div
                        className="flex h-12 items-center justify-center rounded-md border border-[#dfe4da] dark:border-zinc-800"
                        style={{ background: cellColor(v) }}
                        title={`${vendor} · ${r} · ${i} → ${pct(v)}`}
                      >
                        <span className="font-mono text-[11px] font-medium text-[#18201b] dark:text-zinc-100">{v > 0 ? pct(v) : "—"}</span>
                      </div>
                    </td>
                  );
                })}
      ...
      <p className="mt-2 text-[11px] text-[#697362] dark:text-zinc-500">
