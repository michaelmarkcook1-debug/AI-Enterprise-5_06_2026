...
export default function ExportPackLinks({ href, label = "Export procurement pack" }: { href: string; label?: string }) {
  ...
  const linkClass =
    "rounded-full border border-[#d6c9a8] px-2.5 py-1 text-[11px] font-medium text-[#4c5d75] hover:bg-white dark:border-[#2a4a6b] dark:text-[#a7bacd] dark:hover:bg-[#0c2238]";
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      <span className="text-[11px] text-[#5e6b7e] dark:text-[#a7bacd]">{label}</span>
