...
export default function IngestionCostPanel({ vendorCount }: { vendorCount: number }) {
  ...
  const num = (label: string, value: number, set: (n: number) => void, step = 1) => (
    <label className="flex items-center justify-between gap-3 text-xs text-[#3a4a63] dark:text-[#c2d1e0]">
      <span>{label}</span>
      <input
        type="number"
        min={0}
    ...
    <div className="space-y-5">
      ...
      <details className="rounded-lg border border-[#e6dcc3] p-3 dark:border-[#1d3a57]">
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-2">
          ...
          <label className="flex items-center justify-between gap-3 text-xs text-[#3a4a63] dark:text-[#c2d1e0] md:col-span-2">
      ...
      <div>
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-[#5b6b7f] dark:text-[#8fa5bb]">
          Staged model strategy
        </h3>
        <table className="w-full text-left text-xs">
          <thead>
