    ...
    <div className="grid gap-6 lg:grid-cols-[280px_1fr] lg:items-center">
      ...
      <ol className="grid grid-cols-1 gap-y-1.5 sm:grid-cols-2">
        {rows.map((row, i) => (
          <li
            key={row.vendor}
            className="flex items-center justify-between gap-3 rounded-md px-2 py-1 text-sm hover:bg-[#f3ead2] dark:hover:bg-[#143049]"
          >
            <span className="flex items-center gap-2 truncate">
              <span className="w-4 text-right font-mono text-[11px] text-[#5b6b7f] dark:text-[#8fa5bb]">{i + 1}</span>
              <span className="h-3 w-3 flex-none rounded-sm" style={{ background: colorFor(row.vendor) }} />
              <span className="truncate font-medium text-[#13294b] dark:text-[#eef3f8]">{row.vendor}</span>
            </span>
            <span className="font-mono font-semibold text-[#13294b] dark:text-[#eef3f8]">{pct(row.share)}</span>
          </li>
        ))}
      </ol>
    </div>
