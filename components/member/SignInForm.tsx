...
export default function SignInForm({ track, returnTo }: { track?: string; returnTo?: string }) {
  ...
  if (state === "sent") {
    return (
      <div>
        ...
        <p className="mt-1 text-sm text-[#123d2c]/70 dark:text-[#eef3f8]/70">
  ...
  return (
    <form onSubmit={onSubmit}>
      ...
      />
      <label className="mt-3 flex items-start gap-2 text-xs text-[#123d2c]/70 dark:text-[#eef3f8]/70">
        <input
          type="checkbox"
          checked={consent}
      ...
      <button
        ...
        className="mt-3 w-full rounded-lg bg-[#123d2c] px-4 py-2 text-sm font-medium text-white disabled:opacity-60 dark:bg-[#eef3f8] dark:text-[#081410]"
      >
        {state === "loading" ? "Sending…" : "Email me a sign-in link"}
      </button>
      {state === "error" && <p className="mt-2 text-xs text-rose-600 dark:text-rose-400">{msg}</p>}
      <p className="mt-2 text-xs text-[#123d2c]/50 dark:text-[#eef3f8]/50">
