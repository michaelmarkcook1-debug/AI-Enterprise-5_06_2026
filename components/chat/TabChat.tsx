  ...
  return (
    <>
      ...
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className={`fixed bottom-5 right-5 z-40 inline-flex h-11 items-center justify-center gap-0 overflow-hidden whitespace-nowrap rounded-full border border-[#d4af37]/60 bg-[#0b2519] text-sm font-semibold text-[#f6f1e3] shadow-lg transition-[width,gap,padding-left,padding-right] duration-200 ease-out hover:bg-[#123d2c] motion-reduce:transition-none ${
      ...
      {open && (
        <div
          id="tab-chat-panel"
          className="fixed bottom-20 right-5 z-40 flex max-h-[70vh] w-[min(420px,calc(100vw-2.5rem))] flex-col overflow-hidden rounded-xl border border-black/15 bg-white shadow-2xl dark:border-white/15 dark:bg-[#0b2519]"
          role="dialog"
        ...
        >
              ...
              <div key={i} className={m.role === "user" ? "text-right" : ""}>
                <div
                  className={`inline-block max-w-[92%] rounded-lg px-3 py-2 text-left text-xs leading-5 ${
                    m.role === "user"
                      ? "bg-[#123d2c] text-white dark:bg-[#d4af37] dark:text-[#0b2519]"
                      : m.error
          ...
          >
            ...
            <button
              ...
              className="rounded-md bg-[#123d2c] px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-40 dark:bg-[#d4af37] dark:text-[#0b2519]"
            >
              Ask
            </button>
          </form>
